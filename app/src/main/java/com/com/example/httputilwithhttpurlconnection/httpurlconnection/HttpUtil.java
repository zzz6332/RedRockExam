package com.com.example.httputilwithhttpurlconnection.httpurlconnection;
    ...
    public static void getValue() {
        ...
        for (int i = 0; i < need_list.size(); i++) {
            ...
            while (data.indexOf(need) != -1 && !empty) {
                if (data.charAt(data.indexOf(need)-1)== '"' && data.charAt(data.indexOf(need)+need.length())== '"'){
                    ...
                    for (int j = index; j < data.length(); j++) {
                        ...
                        date_buffer.delete(data.indexOf(need),data.indexOf(need)+need.length());
                        data = date_buffer.toString();
                    }
                    }
